...
class TestForecastListView:
    ...
    def test_forecast_list_triggers_refresh_fallback(self, client, monkeypatch):
        session = client.session
        loc = Location.objects.create(name='R1', is_enabled=True, latitude=Decimal('1.0'), longitude=Decimal('2.0'))
        ...
        monkeypatch.setattr('weather.services.SyncWeatherService.update_forecasts_for_location', lambda *_: (_ for _ in ()).throw(Exception('fail')))
        ...
        def fake_refresh(location):
            ...
            DailyForecast.objects.update_or_create(
                ...
                period_start=timezone.make_aware(datetime.combine(today, time(6, 0))),
                period_end=timezone.make_aware(datetime.combine(today, time(18, 0))),
                defaults={'is_daytime': True, 'temperature': 70, 'short_forecast': 'OK', 'wind_speed': 3}
            )
            return True
        monkeypatch.setattr('weather.views._refresh_forecasts_for_location', fake_refresh)
    ...
    def test_forecast_list_context_grouping_and_sorting(self, client):
        session = client.session
        l1 = Location.objects.create(name='Home', is_enabled=True, is_current_location=True, location_type='home')
        ...
        for loc in (l1, l2):
            for day in (today, tomorrow):
                ...
                DailyForecast.objects.create(
                    ...
                    forecast_date=day,
                    period_start=timezone.make_aware(datetime.combine(day, time(18, 0))),
                    period_end=timezone.make_aware(datetime.combine(day, time(23, 59, 59))),
                    is_daytime=False,
                    temperature=60,
        ...
        resp = client.get(reverse('weather:forecast-list'))
        assert resp.status_code == 200
        assert 'grouped_by_date' in resp.context and len(resp.context['grouped_by_date']) >= 2
    ...
    def test_refresh_forecasts_exception_path(self, client, monkeypatch):
        ...
        session = client.session
        loc = Location.objects.create(name='Fail', is_enabled=True, latitude=Decimal('1.0'), longitude=Decimal('2.0'))
        session['location_ids'] = [str(loc.id)]
        session.save()
        loc.last_forecast_update = None
        loc.save(update_fields=['last_forecast_update'])
...
class TestLocationListView:
    def test_location_list_fetches_current_conditions(self, client, monkeypatch):
        ...
        session = client.session
        loc = Location.objects.create(name='Stale', latitude=Decimal('10.0'), longitude=Decimal('20.0'))
        session['location_ids'] = [str(loc.id)]
        session.save()
...
class TestLocationDetailView:
    def test_location_detail_triggers_update_and_alerts(self, client, monkeypatch):
        loc = Location.objects.create(name='Needs Update', latitude=Decimal('10.0'), longitude=Decimal('20.0'))
        # force update needed
        loc.last_forecast_update = None
        loc.save(update_fields=['last_forecast_update'])
        ...
        monkeypatch.setattr('weather.views.fetch_current_conditions', lambda _location: True)
        ...
        forecast_payload = {
            'properties': {
                'periods': [{
                    ...
                    'shortForecast': 'Sunny', 'detailedForecast': 'Clear day',
        ...
        alerts_payload = {'features': [
            {'properties': {
                'id': 'DL1', 'event': 'Storm', 'headline': 'Heads', 'description': 'desc',
                'severity': 'Moderate', 'urgency': 'Expected',
                'onset': f'{today}T07:00:00Z', 'expires': f'{today}T08:00:00Z'
            }}
        ]}
