...
class TestBulkForecastAPI:
    ...
    def test_bulk_forecast_with_nonexistent_locations(self, api_client):
        ...
        response = api_client.post('/api/bulk-forecast/', {'location_ids': [fake_uuid]}, format='json')
    ...
    def test_bulk_forecast_success_with_alerts(self, api_client):
        ...
        WeatherAlert.objects.create(
            location=loc, nws_alert_id='BULK1', event='Advisory',
            headline='Heads up', description='', severity='moderate',
            urgency='expected', onset=timezone.now(),
            expires=timezone.now() + timedelta(hours=2),
            is_active=True, raw_data={},
        )
        payload = {'locations': ['Bulk City'], 'forecast_type': 'both', 'days': 1, 'include_alerts': True}
...
class TestLocationActions:
    ...
    def test_forecasts_get_daily_and_hourly(self, api_client):
        ...
        today = timezone.now().date()
        DailyForecast.objects.create(
            location=loc, forecast_date=today,
            period_start=timezone.make_aware(datetime.combine(today, time(6, 0))),
            period_end=timezone.make_aware(datetime.combine(today, time(18, 0))),
            is_daytime=True, temperature=70, short_forecast='Sunny', wind_speed=5,
        )
    ...
    def test_forecasts_post_create_custom(self, api_client):
        ...
        add_location_to_session(api_client, loc)
        payload = {'date': timezone.now().date().isoformat(), 'is_daytime': True,
                   'temperature': 65, 'short_forecast': 'Partly Cloudy'}
    ...
    def test_alerts_list(self, api_client):
        ...
        add_location_to_session(api_client, loc)
        WeatherAlert.objects.create(
            location=loc, event='Heat Advisory', severity='moderate', urgency='expected',
            headline='Heat', description='Hot', onset=timezone.now(),
            expires=timezone.now() + timedelta(hours=1), is_active=True, raw_data={},
        )
    ...
    def test_ensure_browser_location_updates_existing_current(self, api_client):
        ...
        Location.objects.create(name='Old', latitude=Decimal('30.0'), longitude=Decimal('-95.0'), is_current_location=True)
    ...
    def test_reorder_exception_handling(self, api_client):
        ...
        with patch('weather.models.Location.objects.filter', side_effect=Exception('db error')):
...
class TestUpdateForecastAction:
    ...
    def test_update_forecast_with_current_conditions(self, api_client):
        ...
        location = Location.objects.create(name="Test Location", latitude=Decimal("45.5152"), longitude=Decimal("-122.6784"))
        add_location_to_session(api_client, location)

        mock_grid = Mock(status_code=200)
        mock_grid.json.return_value = {
            'properties': {'gridId': 'PQR', 'gridX': 100, 'gridY': 50,
        ...
        mock_stations.json.return_value = {'features': [{'properties': {'stationIdentifier': 'KPDX'}}]}
        mock_obs = Mock(status_code=200)
        mock_obs.json.return_value = {
            'properties': {'temperature': {'value': 20.5}, 'textDescription': 'Partly Cloudy',
                         'relativeHumidity': {'value': 65}, 'windSpeed': {'value': 16.09},
                         'windDirection': {'value': 315}, 'timestamp': '2025-11-19T14:30:00Z'}
        }
        mock_forecast = Mock(status_code=200)
        mock_forecast.json.return_value = {'properties': {'periods': []}}

        with patch('requests.get', side_effect=[mock_grid, mock_stations, mock_obs, mock_forecast]):
            response = api_client.post(f'/api/locations/{location.pk}/update_forecast/')
    ...
    def test_update_forecast_processes_alerts(self, api_client):
        ...
        location = Location.objects.create(name="Alerts Loc", latitude=Decimal("35.0"), longitude=Decimal("-90.0"))
        add_location_to_session(api_client, location)

        mock_grid = Mock(status_code=200)
        mock_grid.json.return_value = {
            'properties': {'gridId': 'MEG', 'gridX': 10, 'gridY': 20,
        ...
        mock_stations = Mock(status_code=200)
        mock_stations.json.return_value = {'features': [{'properties': {'stationIdentifier': 'KMEM'}}]}
        mock_obs = Mock(status_code=200)
        mock_obs.json.return_value = {
            'properties': {'temperature': {'value': 10}, 'textDescription': 'Cloudy',
                         'relativeHumidity': {'value': 50}, 'windSpeed': {'value': 0},
                         'windDirection': {'value': 180}, 'timestamp': '2025-01-01T00:00:00Z'}
        }
        mock_forecast = Mock(status_code=200)
        mock_forecast.json.return_value = {'properties': {'periods': []}}
        ...
        mock_alerts.json.return_value = {
            'features': [{'properties': {
                'id': 'ALERT1', 'event': 'Warning', 'headline': 'Severe', 'description': 'Desc',
                'severity': 'Severe', 'urgency': 'Immediate',
                'onset': '2025-01-01T01:00:00Z', 'expires': '2025-01-01T02:00:00Z'
            }}]
        }

        with patch('requests.get', side_effect=[mock_grid, mock_stations, mock_obs, mock_forecast, mock_alerts]):
    ...
    def test_update_forecast_creates_periods_with_wind_range(self, api_client):
        ...
        mock_grid = Mock(status_code=200)
        mock_grid.json.return_value = {
            'properties': {'gridId': 'CLE', 'gridX': 5, 'gridY': 6,
        ...
        mock_obs = Mock(status_code=200)
        mock_obs.json.return_value = {
            'properties': {'temperature': {'value': 15}, 'relativeHumidity': {'value': 50},
                         'windSpeed': {'value': 8}, 'windDirection': {'value': 90}, 'timestamp': f'{today}T00:00:00Z'}
        }
        ...
        with patch('requests.get', side_effect=[mock_grid, mock_stations, mock_obs, mock_fcst, mock_alerts]):
            resp = api_client.post(f'/api/locations/{location.pk}/update_forecast/')
        assert resp.status_code == 200
    ...
    def test_update_forecast_geocodes_zip(self, api_client):
        ...
        mock_grid = Mock(status_code=200)
        mock_grid.json.return_value = {
            'properties': {'gridId': 'NYC', 'gridX': 10, 'gridY': 20,
        ...
        with patch('requests.get', side_effect=[mock_geo, mock_grid, mock_stations, mock_fcst, mock_alerts]):
            resp = api_client.post(f'/api/locations/{location.pk}/update_forecast/')
...
class TestExportAPIExtended:
    ...
    def test_export_kml_no_coordinates(self, api_client):
        ...
        response = api_client.post('/api/export/', {'format': 'kml', 'locations': [str(location.id)]}, format='json')
        assert response.status_code == 200
    ...
    def test_export_json_with_forecasts(self, api_client):
        ...
        DailyForecast.objects.create(
            ...
            period_start=timezone.make_aware(datetime.combine(today, time(6, 0))),
            period_end=timezone.make_aware(datetime.combine(today, time(18, 0))),
            is_daytime=True, temperature=70, short_forecast='OK', wind_speed=5,
        )
...
class TestWeatherStatsExtended:
    ...
    def test_weather_stats_with_forecast_data(self, api_client):
        ...
        for i, temp in enumerate([65, 70, 75, 80, 85]):
            DailyForecast.objects.create(
                location=location, forecast_date=today + timedelta(days=i),
                period_start=timezone.make_aware(datetime.combine(today + timedelta(days=i), time(6, 0))),
                period_end=timezone.make_aware(datetime.combine(today + timedelta(days=i), time(18, 0))),
                is_daytime=True, temperature=temp, short_forecast="Test", wind_speed=10,
            )
