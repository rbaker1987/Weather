...
class Location(BaseModel):
    ...
    zip_code: Optional[str] = Field(None, pattern=r'^\d{5}(-\d{4})?$', description="US ZIP code")
...
class WindCondition(BaseModel):
    ...
    direction: Optional[Union[WindDirection, str]] = Field(None, description="Wind direction")
...
class Temperature(BaseModel):
    ...
    unit: TemperatureUnit = Field(default=TemperatureUnit.FAHRENHEIT, description="Temperature unit")
    ...
    def apparent_temperature(self, wind_speed: int) -> int:
        ...
        if self.value > 50 or wind_speed < 5:
            ...
            return int(35.74 + (0.6215 * self.value) - 35.75 * (wind_speed ** 0.16) + 0.4275 * self.value * (wind_speed ** 0.16))


class WeatherCondition(BaseModel):
    """Weather condition description."""
    short_forecast: str = Field(..., description="Brief weather description")
    detailed_forecast: Optional[str] = Field(None, description="Detailed weather description")
...
class DailyForecast(BaseModel):
    ...
    def primary_weather(self) -> Optional[str]:
        ...
        noon_forecasts = [f for f in self.hourly_forecasts if 10 <= f.forecast_time.hour <= 14]
        if noon_forecasts:
...
class WeatherReport(BaseModel):
    ...
    def get_forecasts_for_location(self, location_name: str) -> List[DailyForecast]:
        ...
        return [df for df in self.daily_forecasts if df.location.name.lower() == location_name.lower()]
...
def dict_to_hourly_forecast(data: dict, location: Location) -> HourlyForecast:
    ...
    return HourlyForecast(
        location=location,
        forecast_time=datetime.strptime(f"{data['date']} {data['time']}", "%Y-%m-%d %I%p"),
        ...
        alerts=[WeatherAlert(
            ...
            headline=alert
        ) for alert in data.get('alerts', '').split(', ') if alert]
    )