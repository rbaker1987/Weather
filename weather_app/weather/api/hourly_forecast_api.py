...
class HourlyForecastForLocationAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            if location:
                ...
                ).order_by('period_start')[:hours_count * 2]  # Get extra to ensure coverage
                
                # Build dictionary keyed by rounded hour for fast lookup
                for custom in custom_hourly:
                    # Round to nearest hour for matching
                    rounded_hour = custom.period_start.replace(minute=0, second=0, microsecond=0)
                    ...
                    if rounded_hour not in custom_forecasts_dict:
                        ...
                        existing_diff = abs((custom_forecasts_dict[rounded_hour].period_start - rounded_hour).total_seconds())
                        new_diff = abs((custom.period_start - rounded_hour).total_seconds())
            ...
            for period in periods:
                ...
                if custom_forecast:
                    ...
                    hours_data.append({
                        ...
                        'icon': self._get_weather_icon(custom_forecast.short_forecast, is_daytime),
                        'wind': f"{custom_forecast.wind_speed} mph" if custom_forecast.wind_speed else '',
                    ...
                    hours_data.append({
                        ...
                        'icon': self._get_weather_icon(period.get('shortForecast', ''), is_daytime),
    ...
    def _compute_sunrise_sunset(self, date, lat, lon):
        ...
        try:
            ...
            logger.warning(f"Astral sun calculation failed for {date} at lat={lat}, lon={lon}: {e}. Using fallback.")
            # Fallback: simple approximation (6 AM and 6 PM UTC)
            sunrise_dt = datetime(date.year, date.month, date.day, 6, 0, 0, tzinfo=dt_timezone.utc)
            sunset_dt = datetime(date.year, date.month, date.day, 18, 0, 0, tzinfo=dt_timezone.utc)
            return sunrise_dt, sunset_dt
