    ...
    def handle(self, *args, **options):
        ...
        try:
            if location_ids:
                ...
                self.stdout.write(f"Updating {locations.count()} specified locations...")
                ...
                if not locations.exists():
                    self.stdout.write(
                        self.style.WARNING("No locations need updating")
                    )
                    return
                    ...
                    if verbose:
                        for location_result in result.get('results', []):
                            ...
                            if location_data.get('success'):
                                ...
                                self.stdout.write(f"  {location_name}: {daily} daily, {hourly} hourly")
                            else:
                                error = location_data.get('error', 'Unknown error')
                                self.stdout.write(f"  {location_name}: ERROR - {error}")
                else:
                    self.stdout.write(
                        self.style.ERROR("Bulk update failed")
                    )
